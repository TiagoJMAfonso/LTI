    ...
    <b-container>
      <template>
        ...
        <b-form-select v-model="selected3" :options="options">
          <template slot="first">
            <option :value="null" disabled>Please select an Input Port</option>
          </template>
        </b-form-select>
        <b-form-input
          id="input-2"
          ...
          v-if="visibleRadio"
        ></b-form-input>
        <b-form-input v-model="queue" required placeholder="Please introduce a Queue ID"></b-form-input>
        <b-form-select v-model="selected4" :options="options">
          <template slot="first">
            <option :value="null" disabled>Please select an OutPut Port</option>
          </template>
        </b-form-select>
        <b-form-input v-model="priority" required placeholder="Please introduce a Priority number"></b-form-input>
        ...
        <b-button variant="success" v-on:click.prevent="createQOS(0)">Add Flow</b-button>
...
export default {
  ...
  methods: {
    ...
    createQOS(aux) {
      ...
      if (this.queue == null) {
        this.$toasted.error("Please introduce a Queue ID", {
          duration: 3000,
          position: "top-center",
          theme: "bubble"
        });
      ...
      if (this.queue < 0) {
        this.$toasted.error("Please introduce Queue ID 0 or higher", {
          duration: 3000,
          position: "top-center",
          theme: "bubble"
        });
      ...
      if (this.port == "" && this.selected1 == "") {
        this.$toasted.error(
          "Please select a port name or introduced a port number",
          {
            duration: 3000,
            position: "top-center",
            theme: "bubble"
      ...
      let user = {
        ...
        inputPort: this.selected3,
        priority: this.priority,
        portRadio: this.selected1,
        port: this.port,
        queue: this.queue,
        outputPort: this.selected4
      };
      ...
      axios
        ...
        .then(response => {
          if (this.port == "") {
            if (this.selected3 == 0) {
              this.$toasted.error(
                "Traffic on port " + this.selected1 + " has been dennied",
                {
                  duration: 3000,
                  position: "top-center",
                  theme: "bubble"
              ...
              this.$toasted.success(
                "Traffic on port " + this.selected1 + " has been allowed",
                {
                  duration: 3000,
                  position: "top-center",
                  theme: "bubble"
            ...
            if (this.selected3 == 0) {
              this.$toasted.error(
                "Traffic on port " + this.port + " has been dennied",
                {
                  duration: 3000,
                  position: "top-center",
                  theme: "bubble"
              ...
              this.$toasted.success(
                "Traffic on port " + this.port + " has been allowed",
                {
                  duration: 3000,
                  position: "top-center",
                  theme: "bubble"
